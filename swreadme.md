...
| 层级 | 说明                                                                                |
|------|-----------------------------------------------------------------------------------|
| **误差分析层 <br/>(Evaluation Layer)** | 负责分析不同计算后端推理同一模型时同名张量之间的误差，进而尝试发现RTL层和计算图调度中潜在的精度问题，实现设计迭代闭环。|
| **推理会话层 <br/>(Forwarder Layer)** | 负责创建特定计算图在特定计算后端上的推理会话，实现输入加载、推理执行、中间结果保存和输出验证，提供统一的后端接口。                      |
| **核心抽象层 <br/>(Core Abstraction Layer)** | 基于ONNX标准提供统一的模型和张量管理接口，支持多种后端透明切换。                                  |
...
参考代码路径：./sw/java/main/java/org/forwarder
...
<!--TODO:考虑仅描述Forwarder层的执行流程，主要关注Forwarder层内部的工作流程（白色）和与其他抽象层的交互（别的颜色）-->
包含以下环节：
<!--TODO:麻烦统一一下这张说明图和详细代码介绍的说法，不要自相矛盾-->
1. **加载模型**
...
<!--TODO:建议进一步标出这张图中的每一个组件和其他抽象层中的组件之间的关系，例如：Graph模块提供的组件就被Forwarder层的各种Executor和Dispatcher频繁使用了-->
![1](./graph/Abstraction.png)

---
#### 二. 框架流程
<!--TODO:我很怀疑这个层真的需要这一章介绍么-->
1. **加载模型**
...
##### 8.Prototype
包含OnnxProto3.java和OnnxOperatorsProto3.java两个文件，这两个文件由[onnx标准仓库](https://github.com/onnx/onnx.git)中规定的onnx/onnx.proto3和onnx/onnx-operators.proto3原型文件定义，可以通过protoc编译器编译到java语言的版本，具体可以参考[onnx中的概念](https://onnx.com.cn/onnx/intro/concepts.html)其中的‘使用 protobuf 进行序列化’这一章以及其指向的[protobuf的java语言示例](https://protobuf.dev/getting-started/javatutorial/)。
