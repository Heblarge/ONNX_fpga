error id: file://<WORKSPACE>/hw/spinal/Tiling/Slicer.scala:spinal/core/SInt#resize(+1).
file://<WORKSPACE>/hw/spinal/Tiling/Slicer.scala
empty definition using pc, found symbol in pc: 
found definition using semanticdb; symbol spinal/core/SInt#resize(+1).
empty definition using fallback
non-local guesses:

offset: 9672
uri: file://<WORKSPACE>/hw/spinal/Tiling/Slicer.scala
text:
```scala
package Tiling
...
case class SlicerWrap(slicerCfg: SlicerCfg) extends Component {
  ...
  for(i <- 0 until slicerCfg.systolicArraySideNum){
     := aLanesN(i).asSInt.re@@size(32 bits).asBits:=
  }
