...
case class xpm_memory_sdpram_generic(
  ADDR_WIDTH_A:Int = 10,
  ADDR_WIDTH_B:Int = 10,
   ...
   SIM_ASSERT_CHK:Int = 1,// 0=disable simulation messages 1=enable simulation messages
   ...
   WRITE_PROTECT:Int = 1) extends Generic 
   {
    assert((WRITE_DATA_WIDTH_A>=BYTE_WRITE_WIDTH_A),"WRITE_DATA_WIDTH_A must be bigger then BYTE_WRITE_WIDTH_A")
   }

case class xpm_memory_sdpram(generic: xpm_memory_sdpram_generic) extends BlackBox {
  //BlackBox 中必须要有一个名为generic:Generic的val，否侧将无法生成具有正确参数列表的verilog
  val io = new Bundle {
    val read = new Bundle {
      val clk = in(Bool)
      val rst = in(Bool)
...
case class sdpram_IP_Config(
                        ...
                        READ_DATA_WIDTH_B:Int =16,
                        ADDR_WIDTH_B:    Int = 10){}
case class sdpram_IP(cfg:sdpram_IP_Config) extends Component
{
    val io = new Bundle {
    //write port
    val clka  = in  Bool()
  ...
  val generic=new xpm_memory_sdpram_generic(WRITE_MODE_B=cfg.WRITE_MODE_B,
  ...
  val instance = xpm_memory_sdpram(generic)

    instance.io.write.clk:=io.clka
    instance.io.write.en:=io.ena
    instance.io.write.we:=io.wea.asBits
    instance.io.write.addr<>io.addra
    instance.io.write.din<>io.dina.asBits

    instance.io.read.clk:=io.clkb
    instance.io.read.rst:=False
    instance.io.read.regce:=io.enb
    instance.io.read.en:=io.enb
    instance.io.read.addr<>io.addrb
    instance.io.read.dout.asUInt<>io.doutb
}


object xpm_memory_sdpram_Verilog extends App{
val FileDir = "rtl/xpm_memory_sdpram/verilog"
  import java.io.File
  new File(FileDir).mkdirs()
val cfg=new sdpram_IP_Config()
  SpinalConfig(
    targetDirectory = FileDir,
    oneFilePerComponent = true,
    defaultConfigForClockDomains = ClockDomainConfig(resetActiveLevel = LOW),
    removePruned=false
  ).generateVerilog(new sdpram_IP(cfg)).printPruned()
}
