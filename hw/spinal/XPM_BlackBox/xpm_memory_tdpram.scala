...
case class xpm_memory_tdpram_generic(
  ADDR_WIDTH_A:Int = 6,
  ADDR_WIDTH_B:Int = 6,
  AUTO_SLEEP_TIME:Int = 0,
  BYTE_WRITE_WIDTH_A:Int = 32,
  BYTE_WRITE_WIDTH_B:Int = 32,
...
) extends Generic 
{
    ...
    assert((WRITE_DATA_WIDTH_B>=BYTE_WRITE_WIDTH_B),"WRITE_DATA_WIDTH_B must be bigger then BYTE_WRITE_WIDTH_B")
}

case class xpm_memory_tdpram(generic: xpm_memory_tdpram_generic) extends BlackBox {
  val io = new Bundle {
    val A = new Bundle {
        val clk = in(Bool)
        val rst = in(Bool)
        ...
        val regce = in(Bool) default True
    }
    val B = new Bundle {
        val clk = in(Bool)
        val rst = in(Bool)
        ...
        val write = new Bundle{
            val we = in(Bits(generic.WRITE_DATA_WIDTH_B / generic.BYTE_WRITE_WIDTH_B bits))
            val din = in(Bits(generic.WRITE_DATA_WIDTH_B bits))
        }
        val sbiterr = out(Bool)
        val dbiterr = out(Bool)
        val injectsbiterr = in(Bool) default False
  ...
  addPrePopTask(() => {
    io.flatten.foreach(bt => {
      if (bt.getName().contains("read"))
        bt.setName(bt.getName().replace("read_", "") + "")
      if (bt.getName().contains("write"))
        bt.setName(bt.getName().replace("write_", "") + "")
      if (bt.getName().contains("A"))
        bt.setName(bt.getName().replace("A_", "") + "a")
      if (bt.getName().contains("B"))
        bt.setName(bt.getName().replace("B_", "") + "b")
    })
  })
}
...
case class tdpram_IP_Config(
                        ...
                        WRITE_DATA_WIDTH_B:Int =16,
                        BYTE_WRITE_WIDTH_B:  Int=16,
                        READ_DATA_WIDTH_B:Int =16,
                        ){}
case class tdpram_IP(cfg:tdpram_IP_Config) extends Component
{
val io = new Bundle {
    val A = new Bundle {
        val clk = in(Bool)
        val rst = in(Bool)
        val addr = in(UInt(cfg.ADDR_WIDTH_A bits))
        val read = new Bundle{
            val en = in(Bool)
            val dout = out(Bits(cfg.READ_DATA_WIDTH_A bits))
        }
        val write = new Bundle{
            val we = in(Bits(cfg.WRITE_DATA_WIDTH_A / cfg.BYTE_WRITE_WIDTH_A bits))
            val din = in(Bits(cfg.WRITE_DATA_WIDTH_A bits))
        }
    }
    val B = new Bundle {
        val clk = in(Bool)
        val rst = in(Bool)
        val addr = in(UInt(cfg.ADDR_WIDTH_B bits))
        val read = new Bundle{
            val en = in(Bool)
            val dout = out(Bits(cfg.READ_DATA_WIDTH_B bits))
        }
        val write = new Bundle{
            val we = in(Bits(cfg.WRITE_DATA_WIDTH_B / cfg.BYTE_WRITE_WIDTH_B bits))
            val din = in(Bits(cfg.WRITE_DATA_WIDTH_B bits))
        }
    }
  }
  val generic=new xpm_memory_tdpram_generic(
    ...
    WRITE_MODE_B=cfg.WRITE_MODE_B,
    WRITE_DATA_WIDTH_B=cfg.WRITE_DATA_WIDTH_B,
    BYTE_WRITE_WIDTH_B=cfg.BYTE_WRITE_WIDTH_B,
    READ_DATA_WIDTH_B = cfg.READ_DATA_WIDTH_B
)
  val instance = xpm_memory_tdpram(generic)
  instance.io.A.clk <> io.A.clk
  instance.io.A.rst <> io.A.rst
  instance.io.A.addr <> io.A.addr
  instance.io.A.read <> io.A.read
  instance.io.A.write <> io.A.write
  instance.io.B.clk <> io.B.clk
  instance.io.B.rst <> io.B.rst
  instance.io.B.addr <> io.B.addr
  instance.io.B.read <> io.B.read
  instance.io.B.write <> io.B.write
}


object xpm_memory_tdpram_Verilog extends App{
val FileDir = "rtl/xpm_memory_tdpram/verilog"
  import java.io.File
  new File(FileDir).mkdirs()
val cfg=new tdpram_IP_Config()
  SpinalConfig(
    targetDirectory = FileDir,
    oneFilePerComponent = true,
    defaultConfigForClockDomains = ClockDomainConfig(resetActiveLevel = LOW),
    removePruned=false
  ).generateVerilog(new tdpram_IP(cfg)).printPruned()
}
