...
object xpm_memory_spram_Sim extends App{
val flags = VCSFlags(
    compileFlags = List(
...
//      "+notimingchecks",
  ...
  SimConfig
    ...
    .compile(new spram_IP(cfg)).doSimUntilVoid { dut =>
      //SimTimeout(60000)
      fork {
        dut.io.clka #= false
        while (true) {
      ...
      for (i <- 0 until 10) {
        dut.io.ena #= true
        dut.io.addra #= i    // 从地址 i 读取
        sleep(10)
        val result=dut.io.douta.toInt
