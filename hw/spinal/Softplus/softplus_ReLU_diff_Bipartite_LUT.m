...
%% 定义Softplus函数及参数
f = @(t) (log(1 + exp(t))-max(0,t));   % 目标函数：softplus
...
grid on;
% 初始化最佳结果
best_result = Inf;  % 假设目标是最小化目标函数
best_params = [];   % 存储最佳参数组合
all_results=[];
% 遍历所有可能性
K1=7;
K2=6;
K3=6;
        %K3 = 19 - K1 - K2;  % 确保 K1 + K2 + K3 = 19
        %for g1 = 0:18
            %for g2 = 0:18
            g1=18;
            g2=18;
                fprintf('正在尝试参数: K1=%d, K2=%d, K3=%d, g1=%d, g2=%d\n', K1, K2, K3, g1, g2);
                %% 生成查找表并分析误差
                [log2maxerror, Ptable, Ntable, maxB] = Bipartite_LUT_error_analysis(...
                    f,K1, K2, K3, g1, g2, t_range(1), t_range(2));
                fprintf('最大误差的log2值: %.4f\n', max(log2maxerror));
                current_result = -log2maxerror;
                % 更新最佳结果
                if current_result < best_result
                    best_result = current_result;
                    best_params = [K1, K2, K3, g1, g2];
                end
                %保存参数取值和result结果到allresult
                all_results = [all_results; [K1, K2, K3, g1, g2, current_result]];
...
g1= best_params(4);
g2= best_params(5);
fprintf('最佳参数: K1=%d, K2=%d, K3=%d, g1=%d, g2=%d\n', K1, K2, K3, g1, g2);
...
%% 生成LUT文件（供硬件使用）
file_out = fopen("softplus_ReLU_diff_LUT.txt", 'w');
fprintf(file_out, '// softplus-ReLU LUT: P_table + N_table\n');
