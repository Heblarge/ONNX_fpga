...
object sim_EXP_function_LUT_test extends App {

  // ===========================================================================
  // 0. 环境准备
  // ===========================================================================
  val simReportDir = "rtl/FP8_look_up/sim_EXP_function_test_report"
  ...
  val expx_ref_Queue  = mutable.Queue[Double]()  // 保存期望的参考值
  ...
  while (x_iter.hasNext) {
    ...
    // 1. 解码输入
    // 注意：这里复用 ExponentialFunction 包里的 FP8Utils
    ...
    expx_ref_Queue.enqueue(ref)
  }

  // ===========================================================================
  // 4. 执行仿真
  // ===========================================================================
  module_compiled.doSim("exp_lut_tb", seed = 9527) { dut =>
    SimTimeout(1000000)
    ...
    FlowMonitor(dut.io.expx, dut.clockDomain) { payload =>
      ...
      if (expx_ref_Queue.nonEmpty) {
        var float_ref = expx_ref_Queue.dequeue()
        val input_bits = monitor_x_Queue.dequeue()
        val float_input = FP8Utils.decode(input_bits, cfg)

        // --- 【新增修复逻辑：获取 FP8 最大值】 ---
        val max_fp8_bits = FP8Utils.findMaxFinite(cfg, true)
        val max_fp8_val  = FP8Utils.decode(max_fp8_bits, cfg)

        // 如果参考值超过了 FP8 能表示的极限，我们将“期望参考值”修正为“FP8最大值”
        // 这样可以验证硬件是否正确进行了“饱和处理”，而不是报错
        val ref_is_overflow = !float_ref.isInfinite && !float_ref.isNaN && (float_ref > max_fp8_val)
        
        if (ref_is_overflow) {
             float_ref = max_fp8_val
        }
        ...
        val is_match = (float_received.isNaN && float_ref.isNaN) || 
                       ...
                       (float_received == 0.0 && float_ref == 0.0) // 处理 -0.0 和 +0.0
        ...
        // 打印时，可以在 Log 里标记一下 Overflow
        val statusStr = if (ref_is_overflow) "[SAT]" else "     "

        // 3. 打印详细信息
        println(f"$statusStr In: 0x$input_bits%02X ($float_input%8.4f) => " +
    ...
    dut.clockDomain.waitActiveEdgeWhere(x_Queue.isEmpty && expx_ref_Queue.isEmpty)
...
//     // 只绘制非 NaN 的值。注意：EXP 接受负数输入，所以不限制 x > 0
//     if (!x_val.isNaN) {
...
//   pCompare.title = "FP8 Hardware vs Reference (exp(x))"
//   pCompare.xlabel = "Input x"
//   pCompare.ylabel = "Output exp(x)"
//   pCompare.legend = true
//   fCompare.saveas("tb_EXP_LUT_comparison.png")
...
//   pAbs.xlabel = "Input x"
//   pAbs.ylabel = "Error"
//   fAbs.saveas("tb_EXP_LUT_absolute_error.png")
...
//   pRel.xlabel = "Input x"
//   pRel.ylabel = "Error (%)"
//   fRel.saveas("tb_EXP_LUT_relative_error.png")
  
//   println("Done. Check png files in $simReportDir for results.")
// 
  // ===========================================================================
  // 5. 结果可视化
  // ===========================================================================
  ...
  for (i <- 0 until 256) {
    ...
    if (!x_val.isNaN) {
       // 1. 计算纯数学真值
       val raw_exp = if (x_val.isPosInfinity) Double.PositiveInfinity 
                     else if (x_val.isNegInfinity) 0.0 
                     else Math.exp(x_val)
       
       // 2. 应用饱和逻辑 (与硬件和 Monitor 保持一致)
       // 如果真值超过 FP8 最大值，强制钳位到最大值
       val ref_saturated = if (!raw_exp.isInfinite && !raw_exp.isNaN && raw_exp > max_fp8_val) {
         max_fp8_val
       } else {
         raw_exp
       }
       ...
       // 3. 模拟硬件行为
       // 注意：现在我们要对 ref_saturated 进行编码，而不是 raw_exp
       // 这样避免了 encodeNearest 处理天文数字导致的精度丢失问题
       val hw_bits = FP8Utils.encodeNearest(ref_saturated, cfg)
       val hw_val = FP8Utils.decode(hw_bits, cfg)
       
       if (!hw_val.isNaN && !hw_val.isInfinite) {
         // 计算误差时，也使用饱和后的参考值
         val abs_err = Math.abs(hw_val - ref_saturated)
         val rel_err = if (ref_saturated != 0) abs_err / Math.abs(ref_saturated) * 100 else 0.0
         
         plot_x += x_val
         plot_ref += ref_saturated // 绘图时画的是“期望的饱和曲线”
         plot_out += hw_val
         plot_abs_err += abs_err
         plot_rel_err += rel_err
       }
  ...
  val fCompare = Figure()
  val pCompare = fCompare.subplot(0)
  pCompare += plot(sorted_x, sorted_ref, style = '-', name = "Reference (Saturated)")
  pCompare += plot(sorted_x, sorted_out, style = '.', name = "Output")
  pCompare.title = "FP8 Hardware vs Reference (exp(x))"
  pCompare.xlabel = "Input x"
  pCompare.ylabel = "Output exp(x)"
  pCompare.legend = true
  fCompare.saveas("/tb_EXP_comparison.png") // 建议加上路径
  ...
  pAbs.xlabel = "Input x"
  pAbs.ylabel = "Error"
  fAbs.saveas("/tb_EXP_absolute_error.png")
  ...
  pRel.xlabel = "Input x"
  pRel.ylabel = "Error (%)"
  fRel.saveas("/tb_EXP_relative_error.png")
