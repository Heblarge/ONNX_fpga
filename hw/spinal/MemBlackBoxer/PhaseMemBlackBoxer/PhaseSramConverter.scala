...
class PhaseSramConverter(globalMemVendor: MemVendor = Vendor.Huali, policy: MemBlackboxingPolicy = blackboxAll) extends PhaseMemBlackBoxingWithPolicy(policy) {
  override def doBlackboxing(memTopology: MemTopology): String = {
    ...
    getMemType(memTopology) match {
      ...
      case DualPort =>
        mem.component.rework{
          // dual port may consist that one port want mask control while the other one dont use mask. However, the sram of memory compiler will generate mask for Port A and Port B if one of them need mask.
      ...
      case TwoPort =>
        mem.component.rework {
          ...
          if (wr.mask != null) {
            ...
            ram.io.apa.mask.setAllTo(true) // apa is useless, but Ram1r1w will generate apa.mask input if wr.mask is not null.
