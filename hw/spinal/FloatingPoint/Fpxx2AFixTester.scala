...
object Fpxx2AFixTester extends App {
    ...
    // )
    val flag = VCSFlags (
        // 关键修复：添加 "-debug_access+all"
        compileFlags = List("-kdb", "-lca", "+notimingchecks", "-debug_access+all"), 
        elaborateFlags = List("-fgp", "-kdb", "-lca", "+rad", "+notimingchecks", "-debug_access+all"),
        runFlags = List("-l ./run.log")
    )
...
object FsdbAutoSetup {
  def generate(topModuleName: String): String = {
    val fileName = "auto_fsdb_dump.v"
    
    // 修改处：在最前面加上 `timescale 1ns/1ps
    val content =
      s"""
         |`timescale 1ns/1ps
         |
         |// Auto-generated by SpinalHDL Scala script
         |module fsdb_dumper_auto;
         |  initial begin
         |    $$display("SpinalHDL-Auto: Enabling FSDB Dump for module %s", "$topModuleName");
         |    $$fsdbDumpfile("wave.fsdb");
         |    $$fsdbDumpvars(0, $topModuleName);
         |    $$fsdbDumpflush;
         |  end
         |endmodule
         |""".stripMargin
...
object Fpxx2AFixCompatibleTester extends App {
  ...
  // val myTopName = "Fpxx2AFixCompatible" 
  // val fsdbTriggerFile = FsdbAutoSetup.generate(myTopName)
