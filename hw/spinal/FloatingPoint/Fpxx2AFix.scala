...
class Fpxx2AFixCompatible(
  ...
  val n0 = new Node {
    ...
    val maxShiftVal = intNrBits.value + c.bias + 2 // 预留余量
    val shiftWidth = (c.exp_size + 2).max(log2Up(maxShiftVal) + 1)

    val _shift = SInt(shiftWidth bits)

    _shift := (intNrBits.value - 1 + c.bias) - (U"00" @@ op.exp).asSInt
    val shift     = insert(_shift)
  ...
  val n1 = new Node {
    ...
    val totalBits = intNrBits.value + fracNrBits.value
    val shiftExtended = n0.shift.resize((n0.shift.getWidth.max(log2Up(totalBits + 1) + 1)) bits)

    when(shiftExtended >= totalBits || !n0.ge0) {
      int.clearAll()
      ovfl := False
    }.elsewhen(n0.shift < 0) {
      int  := S(int.getWidth bits, (int.getWidth - 1) -> n0.mant_2c.msb, default -> !n0.mant_2c.msb)
