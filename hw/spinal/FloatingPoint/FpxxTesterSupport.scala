...
object FpxxTesterSupport {
    ...
    def testfloatGen(
        ...
        val genBin = sys.env.get("TESTFLOAT_GEN_BIN") getOrElse {
            assert(Process(Seq("make", "-C", "/root/livehps_1/hw/spinal/FloatingPoint/berkeley-softfloat-3/build/Linux-x86_64-GCC")).! == 0)
            assert(Process(Seq("make", "-C", "/root/livehps_1/hw/spinal/FloatingPoint/berkeley-testfloat-3/build/Linux-x86_64-GCC")).! == 0)

            "/root/livehps_1/hw/spinal/FloatingPoint/berkeley-testfloat-3/build/Linux-x86_64-GCC/testfloat_gen"
        }
