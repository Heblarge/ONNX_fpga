...
class FpxxAddCompatible(o: FpxxAdd.Options) extends Component {
  ...
  val n4 = new Node {
    ...
    // 修复：加宽位宽，防止 E4M3 指数 (15+1=16) 溢出回绕为 0
    val _exp_add_adj  = UInt(o.c.exp_size + 1 bits)
    val _mant_add_adj = UInt(preround_size + 1 bits)
    ...
    when(n3.mant_add(preround_size + 1)) {
      ...
      if (o.stickyBit) _mant_add_adj(0) := n3.mant_add(0) || n3.mant_add(1)
      // Resize 确保加法安全
      _exp_add_adj                      := n0.exp_add.resize(o.c.exp_size + 1) + 1
      _lz.clearAll
    }
      .otherwise {
        _mant_add_adj := n3.mant_add.resize(preround_size + 1)
        _exp_add_adj  := n0.exp_add.resize(o.c.exp_size + 1)
      }
  ...
  val n5 = new Node {
    ...
    // 修复：加宽 SInt 位宽，安全处理 MaxExp + 1 的情况
    val exp_add_m_lz = SInt(o.c.exp_size + 2 bits)

    exp_add_m_lz := n4.exp_add_adj.resize(o.c.exp_size + 2).asSInt - n4.lz
      .resize(o.c.exp_size + 2)
      .asSInt + mant_rounded.msb.asUInt.intoSInt

    val exp_eq_lz = n4.exp_add_adj === n4.lz.resize(o.c.exp_size + 1)
    ...
    when(n0.is_nan) {
      ...
      o.c.inf_encoding match {
        case IEEEInfinity() =>
          // IEEE: 达到 MaxExp (如 15) 即视为溢出 (Inf)
          val maxExpIEEE = (1 << o.c.exp_size) - 1
          is_overflow := n0.is_inf || n4.exp_add_adj.andR || (exp_add_m_lz >= maxExpIEEE)
        case NoInfinity(_) =>
          // FNUZ: 严格大于 MaxExp (如 15) 才视为溢出
          // 因为 MaxExp (15) 是合法的最大普通数值指数
    ...
    if (!o.c.signed_zero) {
      // 保护 NaN 不被误判为 Zero
      when(!n0.is_nan && exp_final === 0 && mant_final === 0) {
