...
object Cli {
    ...
    case class FpxxAdd(@Recurse c: CommonOptions, @Recurse o: FloatingPoint.FpxxAdd.Options) extends Command {};

    case class FpxxConverter(@Recurse c: CommonOptions, @Recurse o: FloatingPoint.FpxxConverter.Options) extends Command {};

    case class FpxxMul(@Recurse c: CommonOptions, @Recurse o: FloatingPoint.FpxxMul.Options) extends Command {};

    case class FpxxAccum(@Recurse c: CommonOptions, @Recurse o: FloatingPoint.FpxxAdd.Options) extends Command {};

    case class FpxxRSqrt(@Recurse c: CommonOptions, @Recurse o: FloatingPoint.FpxxRSqrt.Options) extends Command {};
    ...
    object App extends CommandApp[Command] {
        ...
        def run(command: Command, arg: RemainingArgs): Unit = {
            command match {
                case FpxxAdd(c, o)       => toConfig(c).generate(new FloatingPoint.FpxxAdd(o))
                case FpxxConverter(c, o) => toConfig(c).generate(FloatingPoint.FpxxConverter(o))
                case FpxxMul(c, o)       => toConfig(c).generate(FloatingPoint.FpxxMul(o))
                case FpxxAccum(c, o)     => toConfig(c).generate(FloatingPoint.FpxxAccum(o))
                case FpxxRSqrt(c, o)     => toConfig(c).generate(FloatingPoint.FpxxRSqrt(o))
            }
        }
    }
