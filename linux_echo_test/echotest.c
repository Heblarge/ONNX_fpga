...
int main(int argc, char *argv[])
{
	...
	err_cnt = 0;
	int opt;
	char *rpmsg_dev="virtio0.rpmsg-openamp-demo-channel.-1.1024";
	char *rpmsg_ctrl_dev="virtio0.rpmsg_ctrl.0.0";
	int ntimes = 1;
	char fpath[256];
	...
	if (ret < 0)
		return ret;
	charfd = get_rpmsg_chrdev_fd(rpmsg_ctrl_dev, rpmsg_char_name);
	if (charfd < 0)
		return charfd;
