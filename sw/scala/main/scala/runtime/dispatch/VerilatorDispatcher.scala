...
import Accelerator.{AcceleratorSimInterfaceVerilator, InstJavaTODO}
import runtime.engine._

/**
 * VerilatorDispatcher — HW backend using Verilator RTL simulation.
 *
 * Same logic as VCSDispatcher but uses AcceleratorSimInterfaceVerilator
 * (Verilator) instead of AcceleratorSimInterface (VCS) for much faster simulation.
 *
 * Run via:
 *   sbt 'set Test/javaOptions += "-Dhw.backend=verilator"' 'testOnly runtime.dispatch.HWBackendTestSuite'
 */
class VerilatorDispatcher(dtype: String = "int32") extends AcceleratorDispatcher(dtype) {

  /** Enable online Verilator vs SW cross-check per tile. */
  var crossCheck = true

  /** Maximum allowed per-element error (Verilator vs SW reference). */
  ...
  override def execute(node: GraphNode, inputIds: Seq[String], graph: OnnxGraph): Seq[String] = {
    ...
    if (_tileCount > 0) {
      ...
      println(f"[Verilator] HW#${_hwOpCount} ${node.opType}%-10s '${node.name}' ${_tileCount} tiles ${elapsed}ms $status")
    }
    result
  }
  ...
  override protected def runOneInst(matA: Array[Array[Long]], matB: Array[Array[Long]],
    ...
    val verilatorResult = AcceleratorSimInterfaceVerilator.runSimOneInst(intA, intB, inst)
    val result = verilatorResult.map(_.map(_.toLong))

    // Online cross-check against SW reference
    if (crossCheck) {
      val refResult = AcceleratorSimInterfaceVerilator.runRefOneInst(matA, matB, inst)
      var maxErr = 0L
      var maxErrPos = (0, 0)
      var sumErr = 0L
      var count = 0
      ...
      if (maxErr > crossCheckMaxErr) {
        ...
        println(f"  [XCHK] TILE MISMATCH #${_tileCount} shape=(${inst.input0Shape0},${inst.input0Shape1},${inst.input1Shape1}) " +
          ...
          f"meanErr=$meanErr%.1f maxErr=$maxErr at($ei,$ej) verilator=${result(ei)(ej)} ref=${refResult(ei)(ej)}")
