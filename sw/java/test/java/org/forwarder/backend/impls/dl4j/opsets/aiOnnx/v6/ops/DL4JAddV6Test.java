...
public class DL4JAddV6Test extends DL4JTestCase {
  ...
  @Rule
    ...
    Long axis = 1L;
    Long broadcast = 1L;
		try (INDArray excepted = Nd4j.create(3,4,5);
				INDArray a = Nd4j.create(3,4,5);
				INDArray b = Nd4j.create(1)) {
			this.testAdd(excepted, a, b, axis,broadcast);
			this.testAdd(excepted, b, a, axis,broadcast);
		}
	}
	...
	private void testAdd(INDArray excepted, INDArray a, INDArray b, Long axis, Long broadcast) throws Exception {
		try (DL4JSession session = new DL4JSession(null)) {
			DL4JAddV6 operator = new DL4JAddV6();
			INDArray y = operator.add(a, b, axis, broadcast);
