...
import projectname.SoftplusSim;

import java.util.Arrays;

import static org.junit.Assert.assertEquals;

public class SoftplusSimJavaTest {
    @Test
    public void testRunSim() {
        ...
        for (int i = 0; i < testInputs.length; i++) {
            expectedOutputs[i] = Math.log1p(Math.exp(testInputs[i])) ;
        }
        System.out.println("Starting simulation with inputs: " + Arrays.toString(testInputs));

        float[] actualOutputs = SoftplusSim.runSim(testInputs);
