...
public class HWAcceleratedCastV1 extends HWAcceleratedOperator implements CastV1 {
	...
	protected INDArray cast(INDArray t1, org.onnx4j.tensor.DataType toOnnx4jDataType) {
		org.nd4j.linalg.api.buffer.DataType toHWAcceleratedDataType = HWAcceleratedDataTypeHelper.toHWAcceleratedDataType(toOnnx4jDataType);
		if (toHWAcceleratedDataType == null)
			throw new UnsupportedOperationException(
					String.format("Can not convert datatype from \"%s\"(DL4J) to \"%s\"(ONN4J)", t1.dataType(), toOnnx4jDataType.getCode()));
		else {
			HWAcceleratedDataTypeHelper.ensureNotContainUnexceptedType(toHWAcceleratedDataType, UNEXCEPTED_TYPES);
			return this.cast(t1, toHWAcceleratedDataType);
		}
	}
