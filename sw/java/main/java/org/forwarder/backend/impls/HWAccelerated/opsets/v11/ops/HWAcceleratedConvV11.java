...
public class HWAcceleratedConvV11 extends HWAcceleratedOperator implements ConvV11 {

    @Override
    public OperatorOutputs<INDArray> forward(Node node, Inputs inputs) {
        ConvInputsV11<INDArray> in = new ConvInputsV11<>(node, inputs);
        INDArray x = in.getX();
        if (x.rank() != 4) {
            throw new UnsupportedOperationException("HWAcceleratedConvV11 only supports 4D input (NCHW)");
        }
        INDArray w = in.getW();
        INDArray b = in.getB();
