...
module MatrixCache (
  ...
  input  wire [5:0]    io_readA_Address,
  output wire [31:0]   io_readA_Data,
  input  wire          io_writeA_clk,
  input  wire          io_writeA_rst,
  input  wire [3:0]    io_writeA_Wen,
  input  wire          io_writeA_Valid,
  input  wire [5:0]    io_writeA_Address,
  input  wire [31:0]   io_writeA_Data,
  input  wire          io_switchA,
  input  wire          io_dmaDoneA,
  input  wire          io_readB_clk,
  input  wire          io_readB_rst,
  input  wire          io_readB_Valid,
  input  wire [5:0]    io_readB_Address,
  output wire [31:0]   io_readB_Data,
  input  wire          io_writeB_clk,
  input  wire          io_writeB_rst,
  input  wire [3:0]    io_writeB_Wen,
  input  wire          io_writeB_Valid,
  input  wire [5:0]    io_writeB_Address,
  input  wire [31:0]   io_writeB_Data,
  input  wire          io_switchB,
  input  wire          io_dmaDoneB,
  input  wire          io_readC_clk,
  input  wire          io_readC_rst,
  input  wire          io_readC_Valid,
  input  wire [5:0]    io_readC_Address,
  output wire [31:0]   io_readC_Data,
  input  wire          io_writeC_clk,
  input  wire          io_writeC_rst,
  input  wire [3:0]    io_writeC_Wen,
  input  wire          io_writeC_Valid,
  input  wire [5:0]    io_writeC_Address,
  input  wire [31:0]   io_writeC_Data,
  ...
  wire       [31:0]   cacheA_io_read_Data;
  wire                cacheA_io_status;
  wire                cacheA_io_empty;
  wire       [31:0]   cacheB_io_read_Data;
  wire                cacheB_io_status;
  wire                cacheB_io_empty;
  wire       [31:0]   cacheC_io_read_Data;
  ...
  InputMatrixCache cacheA (
    ...
    .io_read_Address  (io_readA_Address[5:0]    ), //i
    .io_read_Data     (cacheA_io_read_Data[31:0]), //o
    .io_write_clk     (io_writeA_clk            ), //i
    .io_write_rst     (io_writeA_rst            ), //i
    .io_write_Wen     (io_writeA_Wen[3:0]       ), //i
    .io_write_Valid   (io_writeA_Valid          ), //i
    .io_write_Address (io_writeA_Address[5:0]   ), //i
    .io_write_Data    (io_writeA_Data[31:0]     ), //i
    .io_switch        (io_switchA               ), //i
    .io_dmaIntr       (io_dmaDoneA              ), //i
    .io_lifeCfg       (lifeCfgRegA[7:0]         ), //i
  ...
  InputMatrixCache cacheB (
    ...
    .io_read_Address  (io_readB_Address[5:0]    ), //i
    .io_read_Data     (cacheB_io_read_Data[31:0]), //o
    .io_write_clk     (io_writeB_clk            ), //i
    .io_write_rst     (io_writeB_rst            ), //i
    .io_write_Wen     (io_writeB_Wen[3:0]       ), //i
    .io_write_Valid   (io_writeB_Valid          ), //i
    .io_write_Address (io_writeB_Address[5:0]   ), //i
    .io_write_Data    (io_writeB_Data[31:0]     ), //i
    .io_switch        (io_switchB               ), //i
    .io_dmaIntr       (io_dmaDoneB              ), //i
    .io_lifeCfg       (lifeCfgRegB[7:0]         ), //i
  ...
  OutputMatrixCache cacheC (
    ...
    .io_read_Address  (io_readC_Address[5:0]    ), //i
    .io_read_Data     (cacheC_io_read_Data[31:0]), //o
    .io_write_clk     (io_writeC_clk            ), //i
    .io_write_rst     (io_writeC_rst            ), //i
    .io_write_Wen     (io_writeC_Wen[3:0]       ), //i
    .io_write_Valid   (io_writeC_Valid          ), //i
    .io_write_Address (io_writeC_Address[5:0]   ), //i
    .io_write_Data    (io_writeC_Data[31:0]     ), //i
  ...
  always @(*) begin
    ...
    case(busCtrl_readAddressMasked)
      8'h0 : begin
        busCtrl_readRsp_data[7 : 0] = lifeCfgRegA;
      end
      8'h04 : begin
        busCtrl_readRsp_data[7 : 0] = lifeCfgRegB;
      end
      8'h08 : begin
  ...
  always @(posedge clk or posedge reset) begin
    if(reset) begin
      ...
      case(busCtrl_writeAddressMasked)
        8'h0 : begin
          if(busCtrl_writeOccur) begin
            lifeCfgRegA <= io_axi_w_payload_data[7 : 0];
          end
        end
        8'h04 : begin
          if(busCtrl_writeOccur) begin
            lifeCfgRegB <= io_axi_w_payload_data[7 : 0];
          end
        end
        default : begin
        end
...
module OutputMatrixCache (
  ...
  input  wire [5:0]    io_read_Address,
  output wire [31:0]   io_read_Data,
  input  wire          io_write_clk,
  input  wire          io_write_rst,
  input  wire [3:0]    io_write_Wen,
  input  wire          io_write_Valid,
  input  wire [5:0]    io_write_Address,
  ...
  wire       [31:0]   matrixCacheInterface_io_read_Data;
  ...
  wire       [6:0]    matrixCacheInterface_io_read_sdpram_Address;
  wire                matrixCacheInterface_io_write_sdpram_clk;
  wire                matrixCacheInterface_io_write_sdpram_rst;
  wire       [3:0]    matrixCacheInterface_io_write_sdpram_Wen;
  wire                matrixCacheInterface_io_write_sdpram_Valid;
  wire       [6:0]    matrixCacheInterface_io_write_sdpram_Address;
  wire       [31:0]   matrixCacheInterface_io_write_sdpram_Data;
  wire       [31:0]   sdpramModel_3_io_read_Data;
  ...
  OutputMatrixCacheInterface matrixCacheInterface (
    ...
    .io_read_Address         (io_read_Address[5:0]                             ), //i
    .io_read_Data            (matrixCacheInterface_io_read_Data[31:0]          ), //o
    .io_write_clk            (io_write_clk                                     ), //i
    .io_write_rst            (io_write_rst                                     ), //i
    .io_write_Wen            (io_write_Wen[3:0]                                ), //i
    .io_write_Valid          (io_write_Valid                                   ), //i
    .io_write_Address        (io_write_Address[5:0]                            ), //i
    .io_write_Data           (io_write_Data[31:0]                              ), //i
    ...
    .io_read_sdpram_Address  (matrixCacheInterface_io_read_sdpram_Address[6:0] ), //o
    .io_read_sdpram_Data     (sdpramModel_3_io_read_Data[31:0]                 ), //i
    ...
    .io_write_sdpram_Wen     (matrixCacheInterface_io_write_sdpram_Wen[3:0]    ), //o
    .io_write_sdpram_Valid   (matrixCacheInterface_io_write_sdpram_Valid       ), //o
    .io_write_sdpram_Address (matrixCacheInterface_io_write_sdpram_Address[6:0]), //o
    .io_write_sdpram_Data    (matrixCacheInterface_io_write_sdpram_Data[31:0]  ), //o
    .clk                     (clk                                              ), //i
    .reset                   (reset                                            )  //i
  );
  SdpramModel sdpramModel_3 (
    ...
    .io_read_Address  (matrixCacheInterface_io_read_sdpram_Address[6:0] ), //i
    .io_read_Data     (sdpramModel_3_io_read_Data[31:0]                 ), //o
    .io_write_clk     (matrixCacheInterface_io_write_sdpram_clk         ), //i
    .io_write_rst     (matrixCacheInterface_io_write_sdpram_rst         ), //i
    .io_write_Wen     (matrixCacheInterface_io_write_sdpram_Wen[3:0]    ), //i
    .io_write_Valid   (matrixCacheInterface_io_write_sdpram_Valid       ), //i
    .io_write_Address (matrixCacheInterface_io_write_sdpram_Address[6:0]), //i
    .io_write_Data    (matrixCacheInterface_io_write_sdpram_Data[31:0]  ), //i
    .clk              (clk                                              ), //i
    .reset            (reset                                            )  //i
  );
...
//InputMatrixCache_1 replaced by InputMatrixCache
...
module InputMatrixCache (
  ...
  wire       [31:0]   inputMatrixCacheInterface_2_io_read_Data;
  ...
  wire       [6:0]    inputMatrixCacheInterface_2_io_read_sdpram_Address;
  wire                inputMatrixCacheInterface_2_io_write_sdpram_clk;
  wire                inputMatrixCacheInterface_2_io_write_sdpram_rst;
  wire       [3:0]    inputMatrixCacheInterface_2_io_write_sdpram_Wen;
  wire                inputMatrixCacheInterface_2_io_write_sdpram_Valid;
  wire       [6:0]    inputMatrixCacheInterface_2_io_write_sdpram_Address;
  wire       [31:0]   inputMatrixCacheInterface_2_io_write_sdpram_Data;
  wire       [31:0]   sdpramModel_3_io_read_Data;
  ...
  InputMatrixCacheInterface inputMatrixCacheInterface_2 (
    ...
    .io_read_Valid           (io_read_Valid                                           ), //i
    .io_read_Address         (io_read_Address[5:0]                                    ), //i
    .io_read_Data            (inputMatrixCacheInterface_2_io_read_Data[31:0]          ), //o
    .io_write_clk            (io_write_clk                                            ), //i
    .io_write_rst            (io_write_rst                                            ), //i
    .io_write_Wen            (io_write_Wen[3:0]                                       ), //i
    .io_write_Valid          (io_write_Valid                                          ), //i
    ...
    .io_read_sdpram_Address  (inputMatrixCacheInterface_2_io_read_sdpram_Address[6:0] ), //o
    .io_read_sdpram_Data     (sdpramModel_3_io_read_Data[31:0]                        ), //i
    ...
    .io_write_sdpram_Wen     (inputMatrixCacheInterface_2_io_write_sdpram_Wen[3:0]    ), //o
    .io_write_sdpram_Valid   (inputMatrixCacheInterface_2_io_write_sdpram_Valid       ), //o
    .io_write_sdpram_Address (inputMatrixCacheInterface_2_io_write_sdpram_Address[6:0]), //o
    .io_write_sdpram_Data    (inputMatrixCacheInterface_2_io_write_sdpram_Data[31:0]  ), //o
    .clk                     (clk                                                     ), //i
    .reset                   (reset                                                   )  //i
  );
  SdpramModel sdpramModel_3 (
    ...
    .io_read_Address  (inputMatrixCacheInterface_2_io_read_sdpram_Address[6:0] ), //i
    .io_read_Data     (sdpramModel_3_io_read_Data[31:0]                        ), //o
    ...
    .io_write_Wen     (inputMatrixCacheInterface_2_io_write_sdpram_Wen[3:0]    ), //i
    .io_write_Valid   (inputMatrixCacheInterface_2_io_write_sdpram_Valid       ), //i
    .io_write_Address (inputMatrixCacheInterface_2_io_write_sdpram_Address[6:0]), //i
    .io_write_Data    (inputMatrixCacheInterface_2_io_write_sdpram_Data[31:0]  ), //i
    .clk              (clk                                                     ), //i
    .reset            (reset                                                   )  //i
  );
...
//SdpramModel_2 replaced by SdpramModel
...
module OutputMatrixCacheInterface (
  ...
  output wire [6:0]    io_read_sdpram_Address,
  input  wire [31:0]   io_read_sdpram_Data,
  output wire          io_write_sdpram_clk,
  output wire          io_write_sdpram_rst,
  output wire [3:0]    io_write_sdpram_Wen,
  output wire          io_write_sdpram_Valid,
  output wire [6:0]    io_write_sdpram_Address,
  output wire [31:0]   io_write_sdpram_Data,
  input  wire          clk,
  input  wire          reset
);
...
//SdpramModel_1 replaced by SdpramModel
...
module SdpramModel (
  ...
  input  wire          io_read_rst,
  input  wire          io_read_Valid,
  input  wire [6:0]    io_read_Address,
  output wire [31:0]   io_read_Data,
  input  wire          io_write_clk,
  input  wire          io_write_rst,
  input  wire [3:0]    io_write_Wen,
  input  wire          io_write_Valid,
  input  wire [6:0]    io_write_Address,
  input  wire [31:0]   io_write_Data,
  input  wire          clk,
  input  wire          reset
);
  ...
  assign _zz_mem_port = (io_write_Address >>> 2'd2);
  always @(posedge clk) begin
    if(io_write_Valid) begin
      mem[_zz_mem_port] <= io_write_Data;
    end
  end
  ...
  assign _zz_io_read_Data = (io_read_Address >>> 2'd2);
...
module InputMatrixCacheInterface (
  ...
  reg        [7:0]    _zz__zz_when_InputMatrixCache_l160;
  reg        [0:0]    wrPtr;
  reg        [0:0]    rdPtr;
  reg                 bankValid_0;
  reg                 bankValid_1;
  ...
  wire       [7:0]    _zz_when_InputMatrixCache_l160;
  wire       [1:0]    _zz_4;
  wire                _zz_5;
  wire                _zz_6;
  ...
  assign when_InputMatrixCache_l160 = (8'h01 < _zz_when_InputMatrixCache_l160);
  assign _zz_lifeCnt_0 = (_zz_when_InputMatrixCache_l160 - 8'h01);
